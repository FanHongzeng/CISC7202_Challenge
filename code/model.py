...
class three_view_net2(nn.Module):
    def __init__(self, class_num, droprate, stride=2, pool='avg', share_weight=False, VGG16=False, circle=False):
        super(three_view_net, self).__init__()
        self.circle = circle
        self.model_1 = self._initialize_model(class_num, stride, pool, VGG16)
        self.model_2 = self._initialize_model(class_num, stride, pool, VGG16)
        self.model_3 = self.model_1 if share_weight else self._initialize_model(class_num, stride, pool, VGG16)
        self.classifier = ClassBlock(2048 if pool != 'avg+max' else 4096, class_num, droprate, return_f=circle)

    def _initialize_model(self, class_num, stride, pool, VGG16):
        if VGG16:
            return ft_net_VGG16(class_num, stride=stride, pool=pool)
        else:
            return ft_net(class_num, stride=stride, pool=pool)

    def forward(self, x1, x2, x3, x4=None):
        y1 = self._forward_single(x1, self.model_1) if x1 is not None else None
        y2 = self._forward_single(x2, self.model_2) if x2 is not None else None
        y3 = self._forward_single(x3, self.model_3) if x3 is not None else None
        y4 = self._forward_single(x4, self.model_2) if x4 is not None else None
        return (y1, y2, y3, y4) if x4 is not None else (y1, y2, y3)

    def _forward_single(self, x, model):
        x = model(x)
        x = x.view(x.size(0), x.size(1))
